...
"""
    ...
    # 병합 정렬 (Merge Sort)
        - 시간 복잡도
            - 평균 시간 복잡도 : O(n log n)  
                - 병합 정렬은 항상 O(n log n)의 시간 복잡도를 가집니다. 이는 배열을 반으로 나누고, 각각을 재귀적으로 정렬한 후 병합하는 과정에서 발생합니다. 
            - 최악 시간 복잡도 : O(n log n)
                - 병합 정렬은 최악의 경우에도 항상 O(n log n)의 시간 복잡도를 보장합니다. 
            - 최선 시간 복잡도 : O(n log n)
                - 병합 정렬은 최선의 경우에도 O(n log n)의 시간 복잡도를 가집니다.
                
        - 공간 복잡도
            - 공간 복잡도 : O(n)
                - 병합 정렬은 추가적인 메모리 공간이 필요합니다. 각 병합ㅊ 단계에서 임시 배열이 생성되므로, 공간 복잡도는 O(n)가 됩니다. 
        - 요약
            - 퀵 정렬 
                - 시간 복잡도 : 평균 O(n log n), 최악O(n^2), 최선 O(n log n)
                - 공간 복잡도 : O(log n) (최악의 경우 O(n) 스택 공간 필요)
            - 병합 정렬 
                - 시간 복잡도 : 항상 O(n log n)
                - 공간 복잡도 : O(n)
        - 선택 기준 
            - 퀵 정렬은 평균적으로 매우 빠르며, 추가적인 메모리 공간을 거의 사용하지 않기 때문에 메모리 사용이 제한된 환경에서 유리합니다. 그러나 최악의 경우 O(n^2)의 성능을 보일 수 있습니다. 
            - 병합 정렬은 항상 O(n log n)의 시간 복잡도를 보장하며, 안정적인 정렬을 수행합니다. 다만, 추가적인 메모리 공간이 필요하기 때문에 메모리 사용량이 중요한 경우에는 덜 유리할 수 있습니다. 
        - 이 정보를 바탕으로 정렬 알고리즘을 선택할 때 고려할 수 있는 여러 요소들을 이해할 수 있습니다. 
